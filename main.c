...
int main(void)
{
	//declarando punteros de los registos
	...
	uint32_t *ptr_portc_pulldown = PORTC_PULL;
	 ...
	 uint8_t contador = 0;  // Contador para el botón B2
	...
	*ptr_portc_pulldown |= 1 << 1;
    ...
    while(1)
    {
    	...
    	if(*ptr_portc_idr & (1<<0)){
    		...
    		contador++;  //incrementa la variable del contador

    		delay_ms(200); //esperar un tiempo para volver a leer el contador
    	}
